...
var defaultValues={
    ...
    // Global spawner properties
		x:void 0,
		y:void 0,
		speed:0,
		count:1,
		angle:void 0,
		effect_radius:void 0,
		immune:void 0,
	},
	properties:{
    /*
      Just in case if Spacebrook wanted to add those 2 props below.
      Not used for the editor environment because they never added them yet.

      title_color: #F4FAFFFF,
    */
    //
    title_stroke_color: 1113222655,
    // Official Evades.io
		texture:"normal",
		background_color: 0,
		friction:1,
...
function createProperty(name = "name", input = document.createElement("input"), type = "text", options = {}) {
    ...
    if (type === "color") {
        ...
        for (let i in options.selectOptions) {
            ...
            if (options.selectOptions[i][2] === 1)option.setAttribute("class","unused");
            if (options.selectOptions[i][2] === 2)option.setAttribute("class","not-implemented");
            if (options.selectOptions[i][2] === 3)option.setAttribute("class","unused not-implemented");
            if (options.selectOptions[i][2] === 4)option.setAttribute("class","joke");
            if (options.selectOptions[i][2] === 5)option.setAttribute("class","joke not-implemented");
        }
        ...
        addBtn.addEventListener("click",function(){
            options.add(selectOpt[select.value],this);
        });
        li.classList.add(options.selectType ?? "text");
        li.appendChild(select);
